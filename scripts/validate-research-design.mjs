...
const template = read('research/templates/4EB1-retrieve-two-causes.v0.1.0.json');
...
check('Custom maximum of three is rejected; source marks are preserved separately', () => {
  const candidate = structuredClone(template);
...
check('Every pilot task has a unique number, matching AO marks and valid evidence pages', () => {
  assert.equal(new Set(pilot.tasks.map(t => t.id)).size, 11);
  assert.deepEqual(pilot.tasks.map(t => t.number), Array.from({length: 11}, (_, i) => i + 1));
  for (const task of pilot.tasks) {
    assert.equal(Object.values(task.aoMarks).reduce((a,b) => a+b, 0), task.marks);
    assert(task.qpPages.every(p => Number.isInteger(p) && p >= 1 && p <= 36));
    assert(task.msPages.length && task.msPages.every(p => Number.isInteger(p) && p >= 1 && p <= 20));
  }
});
check('All three permitted candidate paths total 100, while all alternatives total 160', () => {
  ...
  assert.equal(pilot.tasks.reduce((s,t) => s+t.marks,0), pilot.counts.allAlternativesMarks);
});
check('Partial extraction cannot be counted as a processed paper', () => {
  assert.equal(pilot.tasks.filter(t => t.detailedExtraction).length, pilot.counts.detailedExtractions);
...
check('Q5, template and exact scheme references agree', () => {
  ...
  assert.equal(source.markScheme.documentId, pilot.documents.find(d => d.type === 'mark-scheme').id);
  assert.deepEqual(source.markScheme.pdfPages, task.msPages);
  assert.equal(task.detailedExtraction.templateId, template.id);
});
check('Raw counts remain discoveries and are not silently replaced by pilot counts', () => {
  ...
  assert.equal(raw.filter(r => r.documentType === 'question-paper' && /extract/i.test(r.title)).length, 2);
...
check('CSV contracts contain unique headers and cover each normalized table', () => {
  const dir = root + 'research/ledger/v1/';
  const files = readdirSync(dir).filter(n => n.endsWith('.csv'));
  assert.equal(files.length, 11);
  for (const name of files) {
    const headers = readFileSync(dir + name, 'utf8').split('\n')[0].split(',');
    assert.equal(new Set(headers).size, headers.length);
    assert(headers.every(h => /^[a-z][a-z0-9_]*$/.test(h)));
  }
});
check('Custom blueprint arithmetic: 22 tasks, 80 marks, only allowed maxima', () => {
  const plan = [[8,2],[10,4],[4,6]];
  assert.equal(plan.reduce((s,[n,m]) => s+n*m,0),80);
  assert.equal(plan.reduce((s,[n]) => s+n,0),22);
  assert(plan.every(([,m]) => [2,4,6].includes(m)));
});

let hashesVerified = 0;
for (const document of pilot.documents) {
  const local = root + pilot.evidenceDirectory + '/' + new URL(document.url).pathname.split('/').pop();
  if (!existsSync(local)) continue;
  assert.equal(createHash('sha256').update(readFileSync(local)).digest('hex'), document.sha256);
  hashesVerified++;
}
console.log(JSON.stringify({scope:'research design only; no API calls, bulk processing or application tests',checks:results.length,results,pilotDocumentHashesVerified:hashesVerified,missingLocalEvidence:3-hashesVerified,activeTemplates:0,fullyProcessedPapers:pilot.counts.fullyProcessedPapers},null,2));
