...
test('library search returns chapter terms and stable deep links', () => {
...
test('library search tolerates a single typo in a long term', () => {
  const results = searchLibrary(physics, 'refracton');
  assert.ok(results.some((result) => result.chapter === 'Waves'));
  assert.ok(results.every((result) => result.href.startsWith('/subjects/physics/')));
});

test('empty and punctuation-only queries are deterministic no-ops', () => {
